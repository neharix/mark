...
let selected_date = JSON.parse(document.querySelector("#date").textContent);
...
function initCalender(monthData) {
	...
	$.each(monthData,
        ...
        classToAdd = "selectable";
			...
			let date = day + "/" + (month.indexOf(document.querySelector(".month").innerHTML) + 1) + "/" + document.querySelector(".year").innerHTML;
			if (date == selected_date) {
				$($($($("tr.days").get(row)).find("td").get(column)).addClass(classToAdd + " " + currentDay + " active").html(day));
			}
			else {
